...
PetscErrorCode ComputeSinusoidalRHS2D(KSP ksp, Vec b, void *user)
{
  Context     *ctx=(Context *)user;
...
PetscErrorCode ComputeSinusoidalRHS3D(KSP ksp, Vec b, void *user)
{
  Context     *ctx=(Context *)user;
  PetscReal    scale=ctx->potential.scale;
  PetscReal    n0=ctx->plasma.n0;
...
PetscErrorCode ComputeFullRHS2D(KSP ksp, Vec b, void *user)
{
  Context         *ctx=(Context *)user;
  PetscReal        dx=ctx->grid.dx;
  PetscReal        dy=ctx->grid.dy;
  ...
  PetscReal        hxx, hyy;
  PetscReal        dndx, dndy;
  PetscReal        dGxdx, dGydy;
  PetscReal        d2ndxx, d2ndyy;
  ...
  for (j=j0; j<j0+nj; j++) {
    for (i=i0; i<i0+ni; i++) {
      ...
      dGxdx = hx*(fluid[j][i+1].Gx - fluid[j][i-1].Gx);
      dGydy = hy*(fluid[j+1][i].Gy - fluid[j-1][i].Gy);
      ...
      Gterm = // (1+kappa^2) (me nue / e) div(flux)
        cG * (dGxdx + dGydy);
      rhs[j][i] = scale * (Eterm + Pterm + Gterm);
    }
  }
...
PetscErrorCode ComputeFullRHS3D(KSP ksp, Vec b, void *user)
{
  Context          *ctx=(Context *)user;
  PetscReal         dx=ctx->grid.dx;
  PetscReal         dy=ctx->grid.dy;
